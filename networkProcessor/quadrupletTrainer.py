...
class QuadrupletTrainer():
	...
	def __init__(self, 
		...
		self.tripletNet = QuadrupletNet(ctx=ctx, margin=margin, margin2=margin2, net=net)
		...
		self.global_step = 0
		self.logger = SummaryWriter(logdir='./log_quadrupletNet') 
	...
	def do_epoch(self, iterations=1000):
		...
		for iteration in range(0, iterations):
			images, coords, px_coords, valid = self.batchSampler.getQuadrupletBatch()
			pred, pos, neg, neg2 = images
			loss = self.tripletNet.train_step(pred, pos, neg, neg2)
			mean_loss += loss

			self.logger.add_scalars('QuadrupletLoss', {
	...
	def validation(self, iterations=20):
		...
		for iteration in range(0, iterations):
			...
			embs = nd.concat(embs, emb_neg2, dim=0)
			val_imgs = nd.concat(val_imgs, val_neg2, dim=0)
...
def unitTest():
	trainer = QuadrupletTrainer(2, 256, ctx=mx.cpu())
