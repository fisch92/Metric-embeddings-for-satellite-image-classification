...
class TiffReader():
	...
	def __init__(self, geomap=GEOMAP.SENTINEL):
		...
		with rasterio.open(self.geomap.value) as geomap:
			self.width = geomap.width
			self.height = geomap.height
			self.transform = geomap.transform
	...
	def readTilePx(self, x, y, width, height):
		with rasterio.open(self.geomap.value) as geomap:
			...
			tile = geomap.read(window=Window(x, y, width, height))
...
def unitTest():
	...
	tiffosm = TiffReader(geomap=GEOMAP.OSM)
